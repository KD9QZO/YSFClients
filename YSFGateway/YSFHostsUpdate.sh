...
YSFHOSTS=/usr/local/share/YSFClients/YSFGateway/YSFHosts.txt
...
if [ ${YSFHOSTSFILEBACKUP} -ne 0 ]; then
...
if [ ${BACKUPCOUNT} -gt ${YSFHOSTSFILEBACKUP} ]; then
	for f in $(ls -tr ${YSFHOSTS}.* | head -${BACKUPSTODELETE}); do
		rm -f $f
	done
fi
